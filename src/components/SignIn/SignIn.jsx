...
const handleSubmit = async (e) => {
    ...
    try {
        ...
        const res = error.response
                if (!res) {
            return setErrorData({message: 'Invalid username or password'})
        }
        if (res.status === 500) {
            setErrorData({ message: 'Something went wrong. Please try again.'})
        } else if (res.status == 401 && res.data.detail) {
        setErrorData({message: res.data.detail})
        } else {
...
return (
    ...
    <form onSubmit={handleSubmit}>
        <div className='form-control'>
            ...
            {errorData.usernamee && <p className='error-message text-red-400 font-bold'>{errorData.username}</p>}
        </div>
